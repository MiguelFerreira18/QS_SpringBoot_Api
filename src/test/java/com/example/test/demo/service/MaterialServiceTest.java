...
class MaterialServiceTest
{
    ...
    @ParameterizedTest
    @CsvSource({"carrinho,true,false,1,1","helicoptero,true,false,2,2"})
    @Order(1)
    @DisplayName("Deve testar se um material e criado na base de dados")
    void testCreateNormalMat(String descricao, boolean isDisponivel, boolean isAvariado, int materialId, int etiquetaId) throws ExecutionException, InterruptedException
    {
    ...
    @ParameterizedTest
    @CsvSource({"materialNeg,true,false,-1,1","materialNeg2,true,false,-2,2"})
    @Order(2)
    @DisplayName("Deve testar se e possivel criar um material com menor que 0")
    void testCreateMatNegativeId(String descricao, boolean isDisponivel, boolean isAvariado, int materialId, int etiquetaId) throws ExecutionException, InterruptedException
    {
        Material m = new Material(descricao,isDisponivel,isAvariado,materialId,etiquetaId);
        String result = myService.createMaterial(m);
        assertNull(result);
    ...
    @ParameterizedTest
    @CsvSource({"um,true,false,11,1","aaaaatxnjevwxmxagqacnklyoxgqigwpfsqujmwagnevakjvxtezitkjxkxrpgayowlxcncftoniumpueqollqmnlodopxfryzqhowesztcbfldwznldncohghvmiooalixf,true,false,12,2",",true,false,13,1",",true,false,14,1"})
    @Order(3)
    @DisplayName("Deve testar se e possivel criar um material com descricao fora dos limites")
    void testCreateMatDescOutOfBonds(String descricao, boolean isDisponivel, boolean isAvariado, int materialId, int etiquetaId) throws ExecutionException, InterruptedException
    {
        Material m = new Material(descricao,isDisponivel,isAvariado,materialId,etiquetaId);
        String result = myService.createMaterial(m);
        assertNull(result);
    ...
    @ParameterizedTest
    @CsvSource({"avariadoDispo,true,true,10,1","avariadoDispo2,true,true,11,2"})
    @Order(4)
    @DisplayName("Deve testar se e possivel criar um material avariado e disponivel")
    void testCreateMatAvariadoDisponivel(String descricao, boolean isDisponivel, boolean isAvariado, int materialId, int etiquetaId) throws ExecutionException, InterruptedException
    {
        Material m = new Material(descricao,isDisponivel,isAvariado,materialId,etiquetaId);
        String result = myService.createMaterial(m);
        assertNull(result);
    ...
    @ParameterizedTest
    @CsvSource({"etiquetaNeg,true,true,10,-1","etiquetaNeg2,true,true,11,-2"})
    @Order(5)
    @DisplayName("Deve testar se e possivel criar um material com etiqueta menor que zero")
    void testCreateMatEtiquetaOutOfBonds(String descricao, boolean isDisponivel, boolean isAvariado, int materialId, int etiquetaId) throws ExecutionException, InterruptedException
    {
        Material m = new Material(descricao,isDisponivel,isAvariado,materialId,etiquetaId);
        String result = myService.createMaterial(m);
        assertNull(result);
    ...
    @ParameterizedTest
    @CsvSource({"carrinhoT1,false,false,0,6","carrinhoT2,true,false,0,6"})
    @Order(7)
    @DisplayName("Deve testar se e possivel criar materiais com o mesmo id")
    void testCreateMatSameId( String descricao, boolean isDisponivel, boolean isAvariado, int materialId, int etiquetaId) throws ExecutionException,InterruptedException
    {
        Material m = new Material(descricao,isDisponivel,isAvariado,materialId,etiquetaId);
